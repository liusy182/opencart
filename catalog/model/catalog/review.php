...
class ModelCatalogReview extends Model {		
	...
	public function getLastReviews($limit = 20) {
		$query = $this->db->query("SELECT r.review_id AS review_id, r.text AS text, p.product_id AS product_id, pd.name AS name, r.date_added FROM " . DB_PREFIX . "review r LEFT JOIN " . DB_PREFIX . "product p ON (r.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.date_available <= NOW() AND p.status = '1' AND r.status = '1' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY r.date_added DESC LIMIT " . (int)$limit);
		return $query->rows;
	}
	
	public function getReviewCounts() {
		$query = $this->db->query("SELECT COUNT(DISTINCT r.review_id) AS total, pd.name AS name FROM " . DB_PREFIX . "review r LEFT JOIN " . DB_PREFIX . "product p ON (r.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.date_available <= NOW() AND p.status = '1' AND r.status = '1' GROUP BY pd.name");
		return $query->rows;
	}
